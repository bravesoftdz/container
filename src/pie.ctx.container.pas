...
uses
  System.TypInfo, System.Generics.Collections, System.SysUtils, System.Rtti, System.Generics.Defaults;
...
type
  ...
  TPieContainer = class(TSingletonImplementation)
  type
    TRegistration = class
      &Interface: PTypeInfo;
      ImplClass: TClass;
  ...
  public
    ...
    procedure RegisterType<TInterface: IInterface; TImpl: class>(const aScope: TScope; const aName: string = ''); overload;
    procedure RegisterType<TInterface: IInterface>(const ADelegate: TActivatorDelegate; const aName: string = ''); overload;
    procedure RegisterType<TInterface: IInterface>(const aScope: TScope; const ADelegate: TActivatorDelegate; const aName: string = ''); overload;
...
begin
  ...
  case LRegistration.Scope of
    ...
    ThreadSingleton:
      LCreateInst := not LRegistration.InstanceByThread.TryGetValue(TThread.Current.ThreadID, AIntf);
  else
    LCreateInst := True;
  end;
  ...
  begin
    ...
    try
      ...
      if (LRegistration.Scope = TScope.ThreadSingleton) then
        LRegistration.InstanceByThread.Add(TThread.Current.ThreadID, LResolvedObj);
